...
def run_lizard(gla_layer, x_gla, q, k, v, local_window, num_meta_tokens):
    """
    Lizard = GLA + AWA
    Combines recurrent (GLA) with sliding window attention (AWA)
    """
    # 1. Run GLA (recurrent attention)
    gla_out = gla_layer(x_gla)
    if isinstance(gla_out, tuple):
        gla_out = gla_out[0]  # Extract output if tuple returned
    
    # 2. Run AWA (sliding window with meta tokens)
    awa_out = run_awa(q, k, v, local_window, num_meta_tokens)
    
    # 3. Fusion: Sum GLA and AWA outputs
    # AWA: [Batch, Heads, Seq, D_head] -> [Batch, Seq, Heads*D_head]
...
if __name__ == "__main__":
    ...
    print(f"\n{'Seq Len':<10} {'GLA (ms)':<12} {'AWA (ms)':<12} {'Lizard (ms)':<12} {'Speedup':<10}")
    print("-" * 80)
    ...
    for seq_len in SEQ_LENS:
        ...
        # Calculate speedup (Lizard vs GLA)
        if t_gla and t_liz:
            speedup = f"{t_gla/t_liz:.2f}x"
        else:
            speedup = "N/A"
        
        print(f"{seq_len:<10} {s_gla:<12} {s_awa:<12} {s_liz:<12} {speedup:<10}")
