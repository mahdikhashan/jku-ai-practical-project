...
def test_correctness():
    ...
    if max_diff > 1e-3:
        ...
        diff = (out_ref - out_triton).abs()
        max_idx = diff.argmax()
        b, h, s, d = torch.unravel_index(max_idx, diff.shape)
        print(f"Largest diff at batch={b}, head={h}, seq={s}, dim={d}")
        print(f"Reference: {out_ref[b, h, s, d]:.6f}")
        print(f"Triton: {out_triton[b, h, s, d]:.6f}")
...
def benchmark():
    ...
    print(f"{'Config':<30} {'Naive (ms)':<15} {'Triton (ms)':<15} {'Speedup':<10}")
    print("-" * 75)
    ...
    for batch, n_heads, seq_len, d_head, window_sizes in configs:
        ...
        print(f"{config_str:<30} {naive_time:<15.3f} {triton_time:<15.3f} {speedup:<10.2f}x")
...
if __name__ == "__main__":
    ...
    test_simple()  # Start with simple case
    test_incremental()  # Find where it breaks
    test_correctness()
    benchmark()
