...
from swa_fzkuji import SWAttention
...
with profile(
    ...
    for _ in range(iterations):
        with record_function("swattn_forward"):
            with torch.no_grad():
                y, _, _ = swattn(x)
    torch.cuda.synchronize()
...
cuda_events = [(e.key, e.device_time) for e in prof.key_averages() 
               if e.device_time > 0 and not e.key.startswith("swattn_forward")]
for name, time_us in sorted(cuda_events, key=lambda x: -x[1])[:5]:
    print(f"  {name[:60]:60s} {time_us/1000:.4f} ms")

prof.export_chrome_trace("swattn_profile.json")

print("\n" + "="*60)
print("COMPARISON")
print("="*60)
print(f"GLA (fused_recurrent): 3.96 ms")
print(f"PyTorch SWA (naive):   4.25 ms")
print(f"SWAttention (Flash):   {avg_device_time_ms:.2f} ms")
