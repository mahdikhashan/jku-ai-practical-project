...
try:
    ...
    # We don't exit here to allow AWA to run if user just wants to test that.
    FLA_AVAILABLE = False
...
if __name__ == "__main__":
    ...
    for seq_len in SEQ_LENS:
        ...
        # throughput = tokens / (time_ms / 1000)
    ...
    ax1.plot(x_gla, y_gla, 'o-', label='GLA (Recurrent)')
    ax1.plot(x_awa, y_awa, 's-', label='AWA (Sliding Win)')
    ax1.set_xlabel('Sequence Length')
    ax1.set_ylabel('Latency (ms)')
    ax1.set_title(f'Latency vs Sequence Length\n(Batch={BATCH_SIZE}, D={D_HEAD})')
    ax1.set_xscale('log')
    ax1.set_yscale('log')
    ...
    ax2.plot(x_gla_tp, y_gla_tp, 'o-', label='GLA (Recurrent)')
    ax2.plot(x_awa_tp, y_awa_tp, 's-', label='AWA (Sliding Win)')
