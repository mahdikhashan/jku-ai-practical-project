...
def benchmark_comparison(args):
    ...
    for seq_len in args.seq_lens:
        ...
        try:
            # Allocate Memory
            q = torch.randn(args.batch_size, args.n_heads, seq_len, args.d_head, device='cuda')
            k = torch.randn(args.batch_size, args.n_heads, seq_len, args.d_head, device='cuda')
            v = torch.randn(args.batch_size, args.n_heads, seq_len, args.d_head, device='cuda')
        ...
        swa_time = measure_kernel_execution(sliding_window_attention, swa_args, args.warmup, args.rep)
        ...
        for num_meta in args.meta_tokens:
            ...
            awa_time = measure_kernel_execution(anchor_window_attention, awa_args, args.warmup, args.rep)
            
            rel_speed = swa_time / awa_time
            print(f"{f'  + {num_meta} meta tokens':<40} {awa_time:<15.4f} {rel_speed:<12.2f}x")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Full Benchmark: Anchor vs Sliding Window Attention")
    ...
    parser.add_argument('--d_head', type=int, default=64, help='Head dimension (default: 64)')
    parser.add_argument('--window_size', type=int, default=64, help='Sliding window size (one-sided) (default: 64)')
    ...
    parser.add_argument('--meta_tokens', nargs='+', type=int, default=[2, 4, 8, 16], 
    ...
    parser.add_argument('--rep', type=int, default=50, help='Measurement repetitions (default: 50)')
    parser.add_argument('--skip_check', action='store_true', help='Skip the initial correctness check')
